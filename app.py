...
def like_page():
    ...
    per_page = 8  # 페이지당 아이템 수
    per_row = 4  # 행당 아이템 수
    row_count = int(per_page / per_row)  # 행 개수 계산
    start_idx = per_page * page
    end_idx = per_page * (page + 1)

    # 데이터베이스에서 좋아요 목록 가져오기
    liked_items = DB.get_liked_items(session.get("id"))  # 로그인한 사용자의 좋아요 목록 가져오기
    item_counts = len(liked_items)  # 전체 아이템 개수
    liked_items = dict(list(liked_items.items())[start_idx:end_idx])  # 현재 페이지에 해당하는 아이템 가져오기
    tot_count = len(liked_items)

    # 행별 데이터 분할
    for i in range(row_count):
        if (i == row_count - 1) and (tot_count % per_row != 0):  # 마지막 행에서 남은 아이템 처리
            locals()[f'data_{i}'] = dict(list(liked_items.items())[i * per_row:])
        else:
            locals()[f'data_{i}'] = dict(list(liked_items.items())[i * per_row:(i + 1) * per_row])

    # 최종 가격 계산
    finalprices = {}
    if liked_items:
        for key, item in liked_items.items():
