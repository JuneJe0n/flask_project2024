...
function updateSelectedOptions() {
    ...
    if (selectedValue) {
        // 새 데이터를 추가
        if (valuelist.includes(selectedText)) {
            const optionDivs = document.querySelectorAll('.newoptionDiv');

            optionDivs.forEach((div) => {
                const optionText = div.querySelector('.optionText');
                
                if (optionText && optionText.textContent.trim() === selectedText) {
                    const quantityInput = div.querySelector('.quantity-input');
                    quantityInput.value = parseInt(quantityInput.value, 10) + 1;
                }
            });
        } else {
            valuelist.push(selectedText);
